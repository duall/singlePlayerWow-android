...
echo "Estimated time: 15-25 minutes (no compilation needed)"
echo ""
...
# Step 4: Clone AzerothCore source (for SQL updates and SourceDirectory)
print_step "Step 4: Downloading AzerothCore source"
...
mariadb -u acore -pacore -e "CREATE DATABASE IF NOT EXISTS acore_world; CREATE DATABASE IF NOT EXISTS acore_characters; CREATE DATABASE IF NOT EXISTS acore_auth; CREATE DATABASE IF NOT EXISTS acore_playerbots;" 2>/dev/null
...
if git clone --filter=blob:none --sparse https://github.com/duall/singlePlayerWow-android.git "$TEMP_CONFIG_DIR"; then
    ...
    if [ -d "configs" ]; then
        find configs -maxdepth 1 -type f -exec cp {} "$SERVER_DIR/etc/" \;
        if [ -d "configs/modules" ]; then
            cp configs/modules/* "$SERVER_DIR/etc/modules/" 2>/dev/null || true
        fi
...
# Copy module .conf.dist files from cloned modules
echo "Copying module configuration files..."
CONF_COUNT=0
find "$SOURCE_DIR/modules" -name "*.conf.dist" | while IFS= read -r conf_file; do
    cp "$conf_file" "$SERVER_DIR/etc/modules/"
    CONF_COUNT=$((CONF_COUNT + 1))
done
print_status "Module .conf.dist files copied from source"

# Ensure every .conf has a matching .conf.dist
...
# Ensure every .conf.dist has a matching .conf
for dist in "$SERVER_DIR/etc/modules/"*.conf.dist; do
    [ -f "$dist" ] || continue
    conf="${dist%.dist}"
    if [ ! -f "$conf" ]; then
        cp "$dist" "$conf"
...
print_status "Module config files synchronized"

# Point SourceDirectory to the cloned source
sed -i "s|SourceDirectory.*=.*|SourceDirectory = \"$SOURCE_DIR\"|" "$SERVER_DIR/etc/worldserver.conf" 2>/dev/null || true
sed -i "s|SourceDirectory.*=.*|SourceDirectory = \"$SOURCE_DIR\"|" "$SERVER_DIR/etc/authserver.conf" 2>/dev/null || true
